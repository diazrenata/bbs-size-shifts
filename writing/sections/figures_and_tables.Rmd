...
biomass_slope_histogram <- ggplot(biomass_slopes_long, aes(biomass_ratio, color = simtype, fill = simtype)) + geom_histogram(boundary = 1) + scale_x_log10() + geom_vline(xintercept = 1) + facet_wrap(vars(simtype), scales = "free_y") + scale_color_viridis_d(option = "plasma", begin = .2, end = .8) + scale_fill_viridis_d(option = "plasma", begin = .2, end = .8) +
  theme(legend.position = "none") + ggtitle("Total biomass") + xlab("")


energy_slope_histogram <- ggplot(energy_slopes_long, aes(energy_ratio, color = simtype, fill = simtype)) + geom_histogram(boundary = 1) + scale_x_log10() + geom_vline(xintercept = 1) + facet_wrap(vars(simtype), scales = "free_y") + scale_color_viridis_d(option = "plasma", begin = .2, end = .8) + scale_fill_viridis_d(option = "plasma", begin = .2, end = .8) +
  theme(legend.position = "bottom") + ggtitle("Total metabolic flux") + xlab("Ratio of last year to first year")

slope_histograms <- multi_panel_figure(columns = 1, height = c(2.25, 2.75), unit = "in") %>%
...
NOT AT ALL CLEAR why I am plotting multiple layers rather than plotting the "winning model" as just the color term. Was I not wanting the "~1"s to overlay on the others? There are fewer of them. 
...
interaction_all_fits <- all_preds %>% group_by_all() %>% filter( grepl( "* source",model_formula), model_family == "Gamma") %>% mutate(plus = substr(model_formula, 12,12) == "+") %>% filter(!plus) %>% ungroup() %>%
  ...
  select(winning_formula, ratio_sim, ratio_real, matssname) %>%
...
decoupling_plot <- ggplot(all_interaction, aes(ratio_sim, ratio_real)) + geom_point(alpha = .2) + geom_point(data = filter(all_interaction, winning_formula != "1"), aes(color = winning_formula), alpha = .3) + scale_y_log10() + scale_x_log10() + scale_color_viridis_d(end = .8) + theme(legend.position = "bottom") + geom_abline(intercept = 0, slope = 1) + geom_vline(xintercept = 1) + geom_hline(yintercept = 1) +
  xlab("Abundance-driven change \n Ratio of last value to first value") + 
