...
```{r, results=T, include=T, fig.dim =c(5, 2.5), eval = F}
...
Here is an example of a real set of "begin" (red) and "end" (blue) ISDs, and the results of a single bootstrap shuffle. For this site (`dat$metadata$location$routename`, `dat$metadata$location$regionname`), the KS test p value is `round(isd_comparison$actual_p[1], 4)`. The observed test statistic has a percentile of `round(isd_comparison$d_percentile[1],2)` and  SES of `round(isd_comparison$d_ses[1], 2)` compared to 500 bootstraps. The observed ISDs have an overlap of `round(isd_comparison$overlap, 2)`.
...
In `sum(all_comps$actual_p < .05)` of `nrow(all_comps)` routes, the KS-test finds a significant difference in the raw ISDs begin-to-end. In `sum(all_comps$d_percentile > .95)` of `nrow(all_comps)`, the percentile score of the KS-test statistic (relative to the bootstrap model) is greater than .95. In `sum(all_comps$d_ses > 2)` of `nrow(all_comps)`, the SES > 2. We proceed as though the "begin" and "end" ISDs are always "significantly" different, and investigate the magnitude and consequences of these differences. 

### Magnitude

```{r, include =T, results = T, eval = F}
...
```{r, include = T, results = T, fig.dim = c(5, 3), eval = F}
...
For `round(100*mean(!grepl("notime", actual_winners[ which(actual_winners$currency == "energy"),]$model)))` percent (for energy use) and `round(100*mean(!grepl("notime", actual_winners[ which(actual_winners$currency == "biomass"),]$model)))` percent (for biomass) of routes, there is a detectable change in abundance and/or one of the currencies from begin to end. 

For `round(100*mean(grepl("full", actual_winners[ which(actual_winners$currency == "energy"),]$model)))` percenat (for energy) and `round(100*mean(grepl("full", actual_winners[ which(actual_winners$currency == "biomass"),]$model)))` percent (for biomass) of routes, there is a *decoupling* of the dynamics of the currency from what would be expected given change in abundance alone.

### Direction of change in abundance

```{r, results = T, include = T, fig.dim = c(5,3), eval = F}
...
```{r, results = T, include = T, fig.dim = c(7,7), eval = F}
...
```{r, results = T, include = T, fig.dim = c(4,3), eval = F}
...
```{r, include = T, results = T, fig.dim =c(4,3), eval = F}
